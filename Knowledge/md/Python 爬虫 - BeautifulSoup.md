## [Python 爬虫 - BeautifulSoup](https://www.runoob.com/python3/python-spider-beautifulsoup.html)
...
* **发送 HTTP 请求**
  ：爬虫通过 HTTP 请求从目标网站获取 HTML 页面，常用的库包括
  `requests`
  。
* **解析 HTML 内容**
  ：获取 HTML 页面后，爬虫需要解析内容并提取数据，常用的库有
  `BeautifulSoup`
  、
  `lxml`
  、
...
BeautifulSoup 提供了多种方法来查找网页中的标签，最常用的包括
find()
和
find\_all()
。
