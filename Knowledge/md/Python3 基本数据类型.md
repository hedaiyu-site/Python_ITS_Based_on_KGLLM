...
[运行实例 »](/try/runcode.php?filename=basic_data_type1&type=python3)
...
**注意:**
在 Python 中，所有非零的数字和非空的字符串、列表、元组等数据类型都被视为 True，只有
**0、空字符串、空列表、空元组**
...
有时候，我们需要对数据内置的类型进行转换，数据类型的转换，你只需要将数据类型作为函数名即可，在下一章节
...
| [tuple(s)](/python3/python3-func-tuple.html) | 将序列 s 转换为一个元组 |
| [list(s)](python3-att-list-list.html) | 将序列 s 转换为一个列表 |
| [set(s)](python-func-set.html) | 转换为可变集合 |
| [dict(d)](python-func-dict.html) | 创建一个字典。d 必须是一个 (key, value)元组序列。 |
