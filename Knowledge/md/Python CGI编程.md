## [Python CGI编程](https://www.runoob.com/python3/python3-cgi-programming.html)
...
| CONTENT\_LENGTH | 当如果服务器通过 POST 方式与 CGI 程序通信，该环境变量表示可从标准输入（STDIN）中读取的有效数据字节数，读取输入数据时必须使用该变量。 |
...
以上实例使用了 Set-Cookie 头信息来设置 Cookie 信息，可选项中设置了 Cookie 的其他属性，如过期时间 Expires，域名 Domain，路径 Path。这些信息设置在
**"Content-type:text/html"**
之前。
